...
constructor(private val repository: CharactersRepository) {
    operator fun invoke(offset: Int, nameStartsWith: String = ""): Flow<Response<List<CharacterItem>>> = flow{
        emit(Response.Loading())
        try {
            val characters = repository.getCharacters(offset, nameStartsWith)
